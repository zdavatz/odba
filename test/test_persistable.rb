...
module ODBA
	...
	class TestPersistable < Test::Unit::TestCase
		...
		class ODBAContainer
		 include ODBA::Persistable
		 attr_accessor	:non_replaceable, :replaceable, :replaceable2, :array
		...
		def test_odba_take_snapshot
			...
			ODBA.cache_server.__next(:store) { 2} 
			ODBA.cache_server.__next(:store)	{	2}
...
=end
		...
		def test_odba_replace_persistables
			...
			ODBA.cache_server.__next(:add_object_connection){|var_id,self_id| }
...
=end
		...
		def test_odba_dump_has_id
			...
			ODBA.cache_server.__next(:store) { |obj, name|
		...
		def test_odba_store_error_raised
			...
			cache_server.__next(:store) { |dump,name|
